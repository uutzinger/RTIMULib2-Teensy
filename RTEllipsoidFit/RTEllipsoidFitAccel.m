...
accelFile = fopen('accelRaw.dta', 'r');
accelCalDataMat = fscanf(accelFile, '%f %f %f', [3, inf]);
fprintf('Mat size = %d\n', size(accelCalDataMat));
fclose(accelFile);

% open output file

corrFile = fopen('accelCorr.dta', 'w');
...
print('-djpeg', 'AccelEllipsoid.tiff')